...
// Funciones de impresi?n aleatoria espec?ficas
void imprimirLibrosAleatoriamente(Libro* libros, int size) {
	random_shuffle(libros, libros + size, [](int n) { return rand() % n; });
	
	cout << setw(30) << left << "Libro" << setw(20) << left << "Autor" << setw(10) << left << "ISBN" << setw(20) << left << "A?o de Publicaci?n"
...
// Nuevas funciones de b?squeda
void busquedaLibro(Libro* libros, int size) {
	string tituloBuscado;
	cout << "Ingrese el t?tulo o parte del t?tulo a buscar: ";
	cin.ignore();
...
int main() {
	...
	Bibliotecario bibliotecarios[] = {
		{101, "Oscar", "Fernandez", "oscarfernandez@gmail.com", "Ciencia Ficci?n"},
	{102, "Mario", "Lazarte", "mariolazarte@gmail.com", "Historia"},
		{103, "Daniel", "Baspineiro", "danielbaspineiro@gmail.com", "Biograf?as"},
	{104, "Leopoldo", "Ruiz", "leopoldoruiz@gmail.com", "Tecnolog?a"},
	...
	do {
		...
		switch (op) {
		case '1':
			// Funci?n para llenar e imprimir datos aleatoriamente
			srand(time(0));
		...
		case '2':
			...
			do {
				...
				case 'X':
					...
					break;
				default:
					cout << "Opci?n inv?lida, por favor elija una opci?n v?lida del submen?." << endl;
				}
